...
pub fn solve(input: &str) -> Solution {
    let (mut grid, (start_x, start_y)) = Grid::from_str(input);
    ...
    let mut current = [(-1, 0), (1, 0), (0, -1), (0, 1)]
        ...
        .filter_map(|(diff_x, diff_y)| {
            if let Some(x) = start_x.checked_add_signed(diff_x) {
                if let Some(y) = start_y.checked_add_signed(diff_y) {
                    Some((x, y))
                } else {
                    None
                }
    ...
    for kind in [
        ...
        match (kind.first(start_x, start_y), kind.second(start_x, start_y)) {
            ...
            while !current.is_empty() {
                while let Some((x, y)) = current.pop() {
                    if let Some(true) = lgrid.get(x as u32, y as u32) {
                        continue;
                    } else if let None = lgrid.get(x as u32, y as u32) {
                        inside = false;
                        continue;
                    }
...
impl Grid {
    pub fn from_str(input: &str) -> (Self, (u32, u32)) {
        let mut width = 0;
        let mut height = 0;
