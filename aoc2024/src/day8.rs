...
pub fn solve(input: &str) -> Solution {
    ...
    let mut antinodes = BitSet::new(width as usize * height as usize);
        ...
        for i in 0..emitters.len() {
            for j in i + 1..emitters.len() {
                ...
                if 0 <= v0_x && v0_x < width && 0 <= v0_y && v0_y < height {
                    antinodes.set((v0_y * width + v0_x) as usize);
                }

                if 0 <= v1_x && v1_x < width && 0 <= v1_y && v1_y < height {
                    antinodes.set((v1_y * width + v1_x) as usize);
                }
            }
        }
    }
        ...
        for i in 0..emitters.len() {
            for j in i + 1..emitters.len() {
                ...
                while 0 <= current_x && current_x < width && 0 <= current_y && current_y < height {
                    antinodes.set((current_y * width + current_x) as usize);
                    current_x += diff_x;
                    current_y += diff_y;
                }
